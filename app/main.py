...
def auto_adjust_brightness_contrast(image):
    ...
    # Adjust brightness based on mean pixel value
    brightness_factor = img_mean / 128.0
    
    # Adjust contrast based on standard deviation
    contrast_factor = 1.0 + (img_std / 128.0)
...
if uploaded_images:
    for uploaded_image in uploaded_images:
        image, image_array = load_and_preprocess_image(uploaded_image)
        ...
        with col2:
            ...
            brightness_factor, contrast_factor = auto_adjust_brightness_contrast(image)
            augmentation_options = {
                'brightness_factor': brightness_factor,
                'contrast_factor': contrast_factor
            }
            
            # Display the brightness and contrast levels
            st.subheader("Automatic Brightness and Contrast Adjustment")
            st.write(f"Brightness Level: {brightness_factor}")
            st.write(f"Contrast Level: {contrast_factor}")
            
            if st.button(f'Classify {uploaded_image.name}'):
                # Apply augmentation and classify the image
        ...
        # Manually adjust augmentation parameters
        st.subheader("Manual Augmentation Adjustment")
        brightness_slider = st.slider("Brightness", min_value=0.5, max_value=2.0, value=brightness_factor, step=0.1)
        contrast_slider = st.slider("Contrast", min_value=0.5, max_value=2.0, value=contrast_factor, step=0.1)
        rotation_angle = st.slider("Rotation Angle", min_value=0, max_value=360, value=0, step=1)
        apply_horizontal_flip = st.checkbox("Apply Horizontal Flip", value=False)
        apply_vertical_flip = st.checkbox("Apply Vertical Flip", value=False)
        apply_random_noise = st.checkbox("Apply Random Noise", value=False)
        
        # Manual augmentation options
        manual_augmentation_options = {
            'brightness_factor': brightness_slider,
        ...
        # Apply manual augmentation and classify the image
        if st.button("Apply Manual Augmentation and Classify"):
            image, image_array = load_and_preprocess_image(uploaded_image, augmentation_options=manual_augmentation_options)
            prediction, _ = predict_image_class(model, image_array)
            st.success(f'Prediction: {str(prediction)}')
